...
namespace Norm.Tests
{
	...
	internal class TestHelper
	{
		...
		public static int TestPort {
			get {
				if (!_testPort.HasValue) {
					_testPort = Int32.Parse (ConfigurationManager.AppSettings["testPort"] ?? "27701");
				}
				return _testPort.Value;
			}
		}
		...
		public static string ConnectionString (string query, string database, string userName, string password)
		{
			...
			return string.Format ("mongodb://{0}{1}:{4}/{2}{3}", authentication, host, database, query, TestPort);
		}
	}
	...
	internal class Session : IDisposable
	{
		...
		public IMongoDatabase DB {
			get { return this._provider.Database; }
		}
		...
		public T MapReduce<T> (string map, string reduce)
		{
			...
			MapReduceResponse response = mr.Execute (new MapReduceOptions (MongoConfiguration.GetCollectionName (typeof(T))) { Map = map, Reduce = reduce });
	...
	internal class ExpandoAddress : IExpando
	{
		...
		public object this[string propertyName] {
			get { return this._properties[propertyName]; }
			set { this._properties[propertyName] = value; }
		}
	...
	public class SuperClassObjectFluentMapped
	{
		...
		{
			MongoConfiguration.Initialize (config => config.For<SuperClassObjectFluentMapped> (c =>
			{
	...
	public class SubClassedObjectFluentMapped : SuperClassObjectFluentMapped
	{
		...
		{
			MongoConfiguration.Initialize (config => config.For<SubClassedObjectFluentMapped> (c => { c.ForProperty (u => u.ABool).UseAlias ("b"); }));
		}
	}
